...
def prot_to_graph(prot, cutoff):
	...
	cadist = th.tensor([dis_matx_ca[i,j] for i,j in zip(*g.edges())]) * 0.1
	dis_matx_center = distance_matrix(g.ndata["center_pos"], g.ndata["center_pos"])
	cedist = th.tensor([dis_matx_center[i,j] for i,j in zip(*g.edges())]) * 0.1
...
def obtain_inter_dist(res, refpos):
	dist_array = distances.distance_array(res.atoms.positions, refpos)
	return dist_array.min(axis=0)


def obtain_inter_graphs(prot, lig, gp=None, gl=None, cutoff=10.0):	
	...
	srcids, dstids = zip(*product(gp.nodes().tolist(),gl.nodes().tolist()))
	atomCoords = lig.GetConformer().GetPositions()
	
	gdata = {('prot', 'pp', 'prot'): gp.edges(),
			('prot', 'pl', 'lig'): (srcids, dstids),			
			('lig', 'll', 'lig'): gl.edges(),
			}
	num_nodes_dict = {'prot': gp.num_nodes(), "lig":gl.num_nodes()}	
	g = dgl.heterograph(gdata, num_nodes_dict=num_nodes_dict)
	g.nodes['prot'].data['feats'] = gp.ndata['feats'].float()
	g.nodes['lig'].data['feats'] = gl.ndata["atom"].float()
	g.edges['pp'].data['feats'] = gp.edata["feats"].float()
	g.edges['ll'].data['feats'] = gl.edata["bond"].float()
	
	ca_inter_dist = distance_matrix(gp.ndata["ca_pos"], atomCoords)
	ce_inter_dist = distance_matrix(gp.ndata["center_pos"], atomCoords)
	cadist = th.tensor(ca_inter_dist[srcids,dstids])
	cedist = th.tensor(ce_inter_dist[srcids,dstids])
	inte_dist = np.array([obtain_inter_dist(res, atomCoords) for res in up.residues])
	mindist = th.tensor(inte_dist[srcids,dstids])
	g.edges['pl'].data['cadist'] = cadist.view(-1,1)
	g.edges['pl'].data['cedist'] = cedist.view(-1,1)
	g.edges['pl'].data['mindist'] = mindist.view(-1,1)
	return g
...
def main():
	...
	#np.save("%s_idsresq.npy"%args.outprefix, ids)
	#save_graphs("%s_presq.bin"%args.outprefix, list(graphs_p))
	#save_graphs("%s_lresq.bin"%args.outprefix, list(graphs_l))
